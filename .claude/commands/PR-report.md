...
## 2-b. 위키(컨텍스트 문서) 갱신 점검

이번 변경이 아래 중 하나에 해당하면, 커밋 전에 관련 위키 문서를 갱신해야 한다.

| 변경 유형 | 갱신 대상 |
|-----------|-----------|
| 새 최상위 폴더/브랜치 추가, 파일 배치 규칙 변경 | `docs/context/MAP.md` |
| 4-layer 흐름 / 데이터 경로 / 새 실행 모드 | `docs/context/architecture.md` |
| 기술 스택 교체, 보안 정책 변경, 트레이드오프가 있는 설계 결정 | `docs/context/decisions.md` (새 ADR 항목 추가, 기존 항목은 *Superseded* 표시) |
| 브랜치 내부 구조/컨벤션 변경 | 해당 `_claude_templates/CLAUDE_*.md` |

- 해당 없음 → "위키 갱신 불요" 한 줄 보고 후 계속.
- 갱신 필요 → 문서를 먼저 수정하고 같은 커밋에 포함. PR 본문의 "변경사항 요약"에 `docs/context/` 갱신 사실을 명시.
