...
| **PR-F** | AI_Agent — `/v1/compose` retrieval 풀에 personal skill 포함 + 시스템 프롬프트 inject 분리 표시 + 격리 가드 테스트 (사용자 A skill 이 사용자 B compose 에 누출 X) | PR-E | AI_Agent |
| **PR-G** | API_Server — `/api/v1/personalization/*` 프록시 + Frontend 가 호출할 형태 정합 | PR-E | API_Server |
...
| 일자 | 작업 | 상태 |
|---|---|---|
| 5/11 (D4) | doc PR + ADR-023 + PR-A (DB 마이그레이션) | — |
| 5/12 (D5) | PR-B (API_Server revision hook) + PR-C (semantic diff) 병렬 | — |
| 5/13 (D6) | PR-D (personalization agent) | — |
| 5/14 (D7) | PR-E (extract_from_diff 라우트) + PR-F (compose 풀 확장 + 격리 가드) | — |
| 5/15 (D8) | PR-G (API_Server 프록시) + PR-H (Frontend UI) 시작 | — |
| 5/16 (D9) | PR-H 완료 + PR-I (Modal smoke 라이브) | — |
| 5/17 (D10) | 영상 시연 캡처 + writeup | — |
| 5/18 (D11) | 제출 (버퍼) | — |
