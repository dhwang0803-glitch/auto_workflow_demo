...
class PersonalMemoryUpsertSkill(BaseModel):
    """One skill row in the personalization memory file (PR-I write side).

    Mirrors `PersonalSkillEntry` but accepts the data over the wire from
    API_Server's activate flow. The embedding is computed server-side
    when omitted (so callers without a BGE-M3 backend can stay simple);
    when provided, it is used as-is so deterministic tests can pin a
    vector without spinning up sentence-transformers.
    """
...
class PersonalMemoryUpsertRequest(BaseModel):
    """POST body for /v1/personalization/memory/upsert."""

    user_id: str = Field(..., min_length=1, max_length=128)
    skill: PersonalMemoryUpsertSkill


class PersonalMemoryUpsertResponse(BaseModel):
    """Result of one upsert.

    `pool_size` is the active count after the write so the caller can
    log it without re-reading the file. `embedding_source` records
    whether the vector was provided by the caller or generated by the
    BGE-M3 backend on the server — useful for diagnosing why a skill
    appears in (or is missing from) the search pool.
    """

    ok: bool = True
    pool_size: int
    embedding_source: Literal["caller", "server"]
