...
6. **Agent 통합** — 시스템 Agent 1종 (`composer-agent`) 으로 등록, 기존 Agent_Management 와 일관

## 2. 범위

**In**
- `API_Server/app/routers/ai_composer.py` — 자연어 → DAG 생성 REST 엔드포인트
- `API_Server/app/services/ai_composer_service.py` — Claude API 호출 + 노드 카탈로그 컨텍스트 주입 + 프롬프트 관리
- Frontend `ChatPanel` — 자연어 입력 + clarification 대화 UI
...
- 테스트:
  - API_Server: Claude API mock 으로 composer_service 단위 테스트
  - Frontend: Playwright 로 "자연어 입력 → draft 수신 → 캔버스 렌더" E2E 1건
...
```json
{
  ...
  "rationale": "LLM 이 제공한 선택 이유 (사용자에게 표시)"
}
```

- **Stateful session**: `session_id` 로 대화 이력 유지 (Redis 또는 in-memory, MVP 는 in-memory)
...
- `src/lib/composer.ts` — `/api/v1/ai/compose` 클라이언트 + SSE 스트리밍 (옵션)
- `src/store/composer.ts` — Zustand slice (session_id, messages, pending diff)
...
- [ ] `POST /api/v1/ai/compose` 엔드포인트 동작 (Claude API mock 으로 테스트)
...
- [ ] Frontend ChatPanel 에서 입력 → 응답 chat 버블 → DAG 가 캔버스에 렌더
...
2. **SSE 스트리밍** — Claude 응답을 stream 할지 여부. MVP 는 단일 응답 대기 (추가 복잡도 회피). 향후 UX 개선으로 추가
