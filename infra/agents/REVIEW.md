...
### 4. Performance / Infra cost & availability (infra branch)
- [ ] Recreation cost/time of resources that `terraform plan` will destroy → recreate
- [ ] Impact of Cloud SQL tier/disk changes on monthly cost (watch ENTERPRISE_PLUS ~$400+/mo)
- [ ] Changing Cloud Run `min_instances > 0` introduces always-on billing
- [ ] Wide-open `authorized_networks` (0.0.0.0/0)
- [ ] cloud-sql-proxy process leak in bash scripts (missing `trap cleanup`)
- **Verdict**: measurable operational/cost degradation → Major, marginal → Minor
